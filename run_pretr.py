...
def clean(path):
    for file, loss in sorted([(file, float(file[path.index('*'):path.index('*')-len(path)+1])) for file in glob.glob(path)], key=lambda x:x[1])[5:]:
...
opt_params = {'model':pretrain, 'dataset':'Coil20', 'n_rand':10, \
              ...
              'weight_init_params':{'epochs_max': 10000, \
                                    ...
                                    'validation_step': 10, \
                                    'stop_criteria': 3}, \
              'space': [Real(1.0E-06, 1.0E-00, "log-uniform", name='lr'), \
